    ...
    elseif regexp(instname,'AMI')
        ...
        switch property
            ...
            case 'fieldc'
                ...
                obj2=GetInst(data,gui,'AMI_Y') ; %This object holds the perp magnet
                ratio = -0.050; %  This is the amount of perpendicular field that is required to compensate for perp element of the parallel field (perp / par)
                
                fprintf(obj,['CONF:FIELD:TARG ',num2str(NM)]);
                fprintf(obj,'RAMP');
                fprintf(obj2,['CONF:FIELD:TARG ',num2str(NM*ratio-0.00)]);
                fprintf(obj2,'RAMP');
                out=query(obj,'FIELD:MAG?');
                meas=str2num(out);
                while abs(NM-meas)>eps
                ...
                out=query(obj2,'FIELD:MAG?');
                meas=str2num(out);
                while abs(NM*ratio-meas)>eps
                    data=guidata(data_object);
                    out=query(obj2,'FIELD:MAG?');
                    meas=str2num(out);
                    pause(2);
